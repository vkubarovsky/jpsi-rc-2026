...
of [10], is however missing a term $(\alpha/\pi)(3r_{max}/2-r_{max}^{2}/4)$, so
that the totals tabulated in Table~I of [10] are low by that amount; at
$r_{max}=1$ the deficit is $5\alpha/4\pi=0.0029$. Our results agree with [10] in
the small-$r_{max}$ rows, where the missing term lies below the quoted precision.
\end{quote}

\section{Please add the closed-form total correction}
...
result~(28). There is no compact closed form for the total ultrarelativistic
correction, which is what an experiment actually applies. We would like it in the
paper. The correct expression --- Eq.~(24) of Ref.~[10] with the missing term
restored --- is
\begin{equation*}
\boxed{\;
...
+\frac{17}{72}+R(r)
+\frac{r^{2}}{16}-\frac{3}{8}r
\right]\;}
\end{equation*}
with
\begin{align*}
r      &= \frac{2\omega_{cut}}{m_V}, &
R(r)   &= \left(r-\frac{r^{2}}{4}-\frac34\right)
          \left[\ln\!\left(\frac{m_V}{m_\ell}\sqrt{1-r}\right)-\frac12\right], &
L_2(x) &= -\int_0^x\frac{\ln(1-t)}{t}\,dt .
\end{align*}
For the muon channel add the electron-loop vacuum polarization, which is exactly
$\delta_{self}$ of Eq.~(8) evaluated with $m_e$.

The last two terms, $r^{2}/16-\tfrac38 r$, are the ones absent from [10].
Checks this expression passes:
\begin{itemize}
\item it reproduces Table~I of this work to all six digits at every $r_{cut}$,
      in both channels, and is independent of $\omega$ by construction;
\item at $r=1$ it gives exactly $\delta_{self}+3\alpha/4\pi$, i.e.\ $\delta_{self}$
      plus the FSIC of Eq.~(16);
\item for $J/\psi\to e^+e^-$ at $r=1$ it gives $0.026135$, matching the
      Bardin--Shumeiko result of Table~III;
\item for $J/\psi\to\mu^+\mu^-$ at $r=1$ it gives $0.034017$ against the exact
      $0.034097$ --- the $8\times10^{-5}$ residue is the ultrarelativistic
      approximation itself, consistent with the accuracy discussion of Section~IV.
\end{itemize}
